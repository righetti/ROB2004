...
class iiwaRobot:
    def __init__(self):
        ...
        T = np.eye(4)
        T[2,3] = 0.1575
        self._joints.append(joint(T, np.array([0,0,1,0,0,0]), None, np.array([0,0,0]), 3.94781))
        
        T = np.eye(4)
        T[2,3] = 0.2025
        self._joints.append(joint(T, np.array([0,1,0,0,0,0]), self._joints[0], np.array([0.0003, 0.042, 0.059]), 4.50275))
        
        T = np.eye(4)
        T[2,3] = 0.2045
        self._joints.append(joint(T, np.array([0,0,1,0,0,0]), self._joints[1], np.array([0, 0.03, 0.13]), 2.45520))
        
        T = np.eye(4)
        T[2,3] = 0.2155
        self._joints.append(joint(T, np.array([0,-1,0,0,0,0]), self._joints[2], np.array([0, -0.034, 0.067]), 2.61155))
        
        T = np.eye(4)
        T[2,3] = 0.1845
        self._joints.append(joint(T, np.array([0,0,1,0,0,0]), self._joints[3], np.array([0.0001, 0.021, 0.076]), 3.41000))
        
        T = np.eye(4)
        T[2,3] = 0.2155
        T[1,3] = -0.0607
        self._joints.append(joint(T, np.array([0,1,0,0,0,0]), self._joints[4], np.array([0, 0.0004, 0.0006]), 3.38795))
        
        T = np.eye(4)
        T[2,3] = 0.0810
        T[1,3] = 0.0607
        self._joints.append(joint(T, np.array([0,0,1,0,0,0]), self._joints[5], np.array([0, 0, 0.02]), 0.35432))
        
        self._T0endeff = np.eye(4)
        self._T0endeff[2,3] = 0.04
...
#         p.setAdditionalSearchPath(pybullet_data.getDataPath())
        ...
        ## add the traybox
        traybox = p.loadURDF("./urdf/traybox.urdf")
        p.resetBasePositionAndOrientation(traybox, [0.35,0.6,0.65], (0., 0., 0., 1.))
        
        cube = p.loadSDF("./urdf/walls.sdf")[0]
        p.resetBasePositionAndOrientation(cube, [0,0.,0.], (0., 0., 0., 1.))
        
        stand = p.loadSDF("./urdf/stand.sdf")[0]
        p.resetBasePositionAndOrientation(stand, [-0.2,0.6,0.5], (0., 0., 0., 1.))
        
        
        stand = p.loadURDF("./urdf/duck_vhacd.urdf")
        p.resetBasePositionAndOrientation(stand, [-0.2,0.6,0.9], (1.57, 0., 0., 1.))
