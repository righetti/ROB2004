...
class NYUDualFingerSimulator:
    def __init__(self, 
        ...
        if xml_path is None:
            self.model = mujoco.MjModel.from_xml_path(
                os.path.join(ASSETS_PATH, 'dual_finger_scene.xml')
            )
        else:
            self.model = mujoco.MjModel.from_xml_path(xml_path)
    ...
    def reset(self, q0=None):
        if q0 is not None:
            q0 = np.array(q0)
            assert q0.shape==(6,), 'Wrong q0 shape! The shape should be (6,)'
            self.data.qpos[:]=q0
        else:
    ...
    def get_cube_positions(self):
        state = dict(
            cube1=self.data.qpos[6:9],
            cube2=self.data.qpos[13:16],
            cube3=self.data.qpos[20:23],
        )
        return state

    def send_joint_torque(self, torque):
        assert np.array(torque).shape == (6,), 'Wrong torque shape! The torque commnand should be a numpy array with shape (6,)'
        self.latest_command_stamp = time.time()
        self.tau_ff[:] = np.array(torque)

    def step(self):
        self.data.ctrl[:6] = self.tau_ff - self.data.qvel[:6]*0.003
        self.step_counter += 1
        mujoco.mj_step(self.model, self.data)
        # Render every render_ds_ratio steps (60Hz GUI update)
        if self.render and (self.step_counter % self.render_ds_ratio) == 0:
    ...
    def getFinger1Pos(self):
        raise NotImplemented('API not implemented.')
        
    def getFinger2Pos(self):
        raise NotImplemented('API not implemented.')